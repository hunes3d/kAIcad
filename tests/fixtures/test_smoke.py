...
from kaicad.schema.plan import AddComponent, Label, Plan, Wire
...
def test_schema_validation():
    ...
    plan_dict = {
        "ops": [
            {"op": "add_component", "ref": "R1", "symbol": "Device:R", "value": "1k", "at": [100, 100], "rot": 0},
            {"op": "label", "net": "VCC", "at": [120, 100]},
            {"op": "wire", "from": "R1:1", "to": "R2:2"},
        ]
    }
...
def test_schema_serialization_uses_aliases():
    ...
    plan = Plan(ops=[Wire(op="wire", from_="R1:1", to="R2:2")])
...
def test_settings_persistence(tmp_path):
    ...
    import kaicad.config.settings as settings_module
    ...
    try:
        ...
        s = Settings(
            openai_model="gpt-4o", openai_temperature=0.5, openai_api_key="test-key", default_project="/test/path"
        )
        s.save()
...
def test_imports():
    ...
    from kaicad.ui.cli import main
    from kaicad.core.planner import plan_from_prompt
    from kaicad.ui.web.app import create_app
