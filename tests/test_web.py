...
from sidecar.web import app, _load_current_project, _save_current_project
...
def test_index_with_invalid_project_path(client):
    ...
    response = client.post('/', data={
        'project_path': '/nonexistent/path/test.kicad_sch',
        'user_prompt': 'Test'
    })
...
def test_json_endpoints_return_json(client):
    ...
    response = client.post('/generate_description', 
                          json={'prompt': 'test'},
