...
from kaicad.core.writer import apply_plan, snap_to_grid
...
def test_plan_schema_version_validation():
    ...
    valid = {"plan_version": 1, "ops": [{"op": "label", "net": "VCC", "at": [10, 10]}]}
    plan = Plan.model_validate(valid)
    assert plan.plan_version == 1

    # Plan without version should use default
    no_version = {"ops": [{"op": "label", "net": "GND", "at": [20, 20]}]}
...
def test_plan_serialization_includes_version():
    ...
    from kaicad.schema.plan import AddComponent

    plan = Plan(ops=[AddComponent(op="add_component", ref="U1", symbol="MCU:ATmega328", value="", at=(100, 100))])
