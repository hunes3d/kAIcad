...
from .schema import Plan, ApplyResult, Diagnostic, PLAN_SCHEMA_VERSION
    ...
    for op in plan.ops:
        if op.op == "add_component":
            ...
            try:
                if hasattr(Symbol, "from_lib"):
                    ...
                    try:
                        ...
                        setattr(sym, "Reference", op.ref)
                        setattr(sym, "Value", op.value)
                    except Exception:
                    ...
                    diagnostics.append(Diagnostic(stage="writer", severity="info", ref=op.ref,
                                                  message=f"Added {op.ref} ({op.symbol})"))
                else:
            ...
            try:
                ...
                if from_pos and to_pos:
                    ...
                    diagnostics.append(Diagnostic(
                        ...
                        severity="warning",
                        ref=from_ref,
                        message=f"Pin coordinates unavailable, using net label fallback",
                    ...
                    try:
                        lf = doc.label
                        l1 = lf.new(); setattr(l1, 'value', net_name); setattr(l1, 'at', (from_x, from_y)); lf.append(l1)
                        l2 = lf.new(); setattr(l2, 'value', net_name); setattr(l2, 'at', (to_x, to_y)); lf.append(l2)
                        affected_refs.extend([from_ref, to_ref])
                        diagnostics.append(Diagnostic(
                            stage="writer",
                            severity="info",
            ...
            try:
                ...
                x, y = snap_to_grid(op.at[0]), snap_to_grid(op.at[1])
                lab = doc.label.new()
                setattr(lab, 'value', op.net)
                setattr(lab, 'at', (x, y))
                doc.label.append(lab)
                diagnostics.append(Diagnostic(
                    stage="writer",
                    severity="info",
                    message=f"Added label '{op.net}' at ({x}, {y})"
                ))
            except Exception as e:
                diagnostics.append(Diagnostic(
                    stage="writer",
